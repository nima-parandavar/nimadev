<template>
  <div class="container mx-auto my-10">
    <min-form :state="state" class="flex flex-col gap-5" @submit="create">
...
const state = reactive({
  ...
  metaKeywords: undefined as string | undefined,
...
async function create() {
  loading.value = true;
  const { status } = await useFetch("/api/articles", {
    method: "POST",
    body: state,
  });
  loading.value = false;
  if (status.value === "success") {
    return await navigateTo(localePath("/panel/articles"));
  }
}
definePageMeta({
  layout: "panel",

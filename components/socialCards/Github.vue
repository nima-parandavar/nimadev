<template>
  <div
    class="bg-black h-full w-full rounded-3xl flex flex-col gap-7 p-5 lg:p-10 justify-between"
  >
    <div class="inline-flex justify-between items-center">
      ...
      <NuxtLink
        to="https://github.com/nima-parandavar"
        target="_blank"
        class="inline-flex items-center justify-center gap-3 text-black"
      >
        <span
          class="bg-white p-5 lg:px-12 py-2 rounded-full inline-flex items-center justify-center gap-1"
    ...
    >
      ...
      >
        <span
          ...
          :class="color ? `sqr-${currentTheme}` : 'bg-white'"
        ></span>
      </div>
    </div>
  </div>
...
const randomlyColorSquares = (_rows: number, _cols: number) => {
  // @ts-ignore
  grid.value = Array(_rows)
    .fill()
    .map(() => Array(_cols).fill(false));
